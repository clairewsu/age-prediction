...
def integrated_gradients_global(model, X_tensor, feat_names, n_samples=IG_SAMPLES, out_dir=OUT_DIR):
    try:
        from captum.attr import IntegratedGradients
    except Exception as e:
        print("Captum not available; skipping IG:", e)
        return None
    model.to(DEVICE); model.eval()
    n = min(n_samples, X_tensor.shape[0])
    X_sample = X_tensor[:n].to(DEVICE)
    baseline = torch.zeros_like(X_sample).to(DEVICE)
    ig = IntegratedGradients(model)
    attr, delta = ig.attribute(X_sample, baselines=baseline, target=None, return_convergence_delta=True)
    attr_np = attr.detach().cpu().numpy()
    mean_abs_attr = np.mean(np.abs(attr_np), axis=0)
    ig_df = pd.DataFrame({'feature': feat_names, 'mean_abs_ig': mean_abs_attr}).sort_values('mean_abs_ig', ascending=False)
    ig_df.to_csv(os.path.join(out_dir, 'integrated_gradients_fullcsv.csv'), index=False)
    print("Saved integrated_gradients_fullcsv.csv")
    return ig_df
...
def main():
    ...
    ig_df = integrated_gradients_global(model, X_tensor, feat_names, n_samples=IG_SAMPLES)
    ig_df.to_csv(os.path.join(OUT_DIR, 'integrated_gradient_interpret.csv'), index=False)
